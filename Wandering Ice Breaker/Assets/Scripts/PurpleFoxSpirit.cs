...
public class PurpleFoxSpirit : Enemy
{
    ...
    public List<gameState> openList = new List<gameState>(); // All the squares that are being considered to find shortest path
    public List<gameState> closedList = new List<gameState>(); // Sqaures that do not have to be considered again

    public bool justWokeUp = true;

    public class gameState
    {
        public gameState(int row, int col, int h, int g, gameState pred)
        {
            enemyRow = row;
            enemyCol = col;
            gScore = g;
    ...
    private void Awake()
    {
        setTimer(0.1f); // Changing the AI Timer, it should take a lot longer for this spirit to get moving
    ...
    override public void Move() // A* Movement 
    {
        ...
        closedList.Add(openList[irecord]); // Move best spot to closed list
        openList.RemoveAt(irecord); // Remove it from the open list
        ...
        if (walkableTiles.Contains(manager.gridStatus[recordBest.enemyRow + 1][recordBest.enemyCol]))
        {
            ...
            {
                if((closedList[i].enemyRow == recordBest.enemyRow + 1) && (closedList[i].enemyCol == recordBest.enemyCol))
                {
                    continueBool = true;
                }
            }
            ...
            if(continueBool == false)
            {
                ...
                for (int i = 0; i < openList.Count; i++)
                {
                    if ((openList[i].enemyRow == recordBest.enemyRow + 1) && (openList[i].enemyCol == recordBest.enemyCol))
                    {
                        continueBool2 = true;
                        irecord2 = i;
                    }
                ...
                {
                    gameState temp2State = openList[irecord2];
                    ...
                    {
                        openList[irecord2].gScore = recordBest.gScore + 1;
                        openList[irecord2].predecessor = recordBest;
                    }
                ...
                } else // not in open list, so we record its scores and add it
                {
                    ...
                    gameState temp2State = new gameState(recordBest.enemyRow + 1, recordBest.enemyCol, newHeuristicScore, recordBest.gScore + 1, recordBest);
                    openList.Add(temp2State);
                }
        ...
        if (walkableTiles.Contains(manager.gridStatus[recordBest.enemyRow][recordBest.enemyCol - 1]))
        {
            ...
            for (int i = 0; i < closedList.Count; i++)
            {
                if ((closedList[i].enemyRow == recordBest.enemyRow) && (closedList[i].enemyCol == recordBest.enemyCol - 1))
                {
                    continueBool = true;
                }
            }
            ...
            if (continueBool == false)
            {
                ...
                for (int i = 0; i < openList.Count; i++)
                {
                    if ((openList[i].enemyRow == recordBest.enemyRow) && (openList[i].enemyCol == recordBest.enemyCol - 1))
                    {
                        continueBool2 = true;
                        irecord2 = i;
                    }
                ...
                else // not in open list, so we record its scores and add it
                {
                    ...
                    gameState temp2State = new gameState(recordBest.enemyRow, recordBest.enemyCol - 1, newHeuristicScore, recordBest.gScore + 1, recordBest);
                    openList.Add(temp2State);
                }
    ...
    public void ResetEverything()
    {
        ...
        // Initial gameState
        gameState tempState = new gameState(row, col, heuristicScore, g, null);
        closedList.Add(tempState);
        ...
        if (walkableTiles.Contains(manager.gridStatus[row + 1][col]))
        {
            ...
            gameState temp2State = new gameState(row + 1, col, newHeuristicScore, g + 1, tempState);
            openList.Add(temp2State);
        }
        if (walkableTiles.Contains(manager.gridStatus[row - 1][col]))
        {
        ...
        if (walkableTiles.Contains(manager.gridStatus[row][col + 1]))
        {
            ...
            gameState temp2State = new gameState(row, col + 1, newHeuristicScore, g + 1, tempState);
            openList.Add(temp2State);
        }
        if (walkableTiles.Contains(manager.gridStatus[row][col - 1]))
        {
